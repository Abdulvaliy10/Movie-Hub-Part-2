...
const MovieModal = ({ open, setOpen }) => {
    ...
    const getSerieDetails = async () => {
        ...
        setLoading(true);
        try {
            const data = await moviePageApi.getSerieById(open.id); 
            setMovieDetail(data);
        } catch (error) {
    ...
    const getSerieCast = async () => {
        if (!open) return;
        try {
            const data = await moviePageApi.getSerieCast(open.id);
            setCast(data.cast || []);
        } catch (error) {
            ...
            <Box sx={{ ...style }}>
                {loading || !movieDetail ? (
                    ...
                    <>
                        ...
                        />
                        <Stack gap={2}>
                            <Typography variant="h4">{movieDetail.original_name}</Typography>
                            <Box
                                sx={{
